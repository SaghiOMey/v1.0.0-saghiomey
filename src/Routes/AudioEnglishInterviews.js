...
export default function AudioEnglishInterviews(props) {
  const englishs = [...props.episodes].reverse().filter(p => p.ln === "en");
  const [ep, setEp] = useState(englishs.slice(0,9));
  // const [episodes, setEpisodes] = useState(englishs.slice(0,9));
    ...
    <div className="relative">
      ...
      <div className="mx-auto max-w-2xl py-16 px-4 sm:py-4 sm:px-6 lg:max-w-7xl lg:px-8">
            ...
            {ep.length > englishs.length ?
                <> 
                {englishs.slice(0,9).map((episode) => ( 
